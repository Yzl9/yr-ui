<template>
  <hsc-window-style-metal>
    <hsc-window
      title="with {min,max}{Width,Height}"
      :resizable="true"
      :minWidth="200"
      :minHeight="200"
      :maxWidth="400"
    ...
    <hsc-window
      title="width,height (sync)"
      :resizable="true"
      :width.sync="width"
      :height.sync="height"
    >
      <p style="padding: 1em;">width={{ width }}, height={{ height }}</p>
    </hsc-window>
    ...
    >
      <iframe
        ...
        :src="`data:text/html;base64,${iframeSrc}`"
        style="width: 100%; height: 100%; box-sizing: border-box; border-style: none;"
      />
    </hsc-window>
      ...
      <table>
        ...
        <tr v-for="i in range(n)" :key="i">
          ...
          <td
            ...
            v-html="hex(((i / n) * j) / n)"
            :style="{
              backgroundColor: `rgb(${Math.floor((255 * i) / n)}, ${Math.floor(
                (255 * j) / n
              )}, 127)`,
            }"
          />
    ...
    <a
      ...
      >https://developer.mozilla.org/en-US/docs/Web/CSS/radial-gradient</a
    >
  </hsc-window-style-metal>
</template>
