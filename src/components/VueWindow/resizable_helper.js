...
export class ResizableHelper {
  constructor(container, options) {
    ...
    let resizeHandleClasses = HandleClasses
    if (Array.isArray(this.options.resizeSiders)) {
      resizeHandleClasses = resizeHandleClasses.filter(resizeHandler =>
        this.options.resizeSiders.includes(resizeHandler.name)
      )
    }
    this.handles = resizeHandleClasses.map(H => new H(container, this))
...
HandleClasses.push(
  ...
  class Left extends HandleBase {
    setPosition(e) {
      let width = this.width0 - (e.clientX - this.x0)
      let left = this.left0 + e.clientX - this.x0
      if (width >= this.helper.options.maxWidth) {
        width = this.helper.options.maxWidth
        left = this.minLeft
      } else if (width <= this.helper.options.minWidth) {
        width = this.helper.options.minWidth
        left = this.maxLeft
      }
    ...
    applyStyle(style) {
      style.left = `${-this.handleSize}px`
      style.bottom = `${this.handleSize}px`
      style.width = `${1 * this.handleSize}px`
